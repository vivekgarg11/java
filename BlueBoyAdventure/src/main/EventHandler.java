...
public class EventHandler {
	...
	public void setDialogue() {
		...
		eventMaster.dialogues[1][0] = "Teleport!";
		eventMaster.dialogues[2][0] = "You drank water.\nYour life and mana have been recovered.\n"
				+ "(The Progress has been saved)";
	}

	public void checkEvent() {

		// Check if player character is one tile away from the last event
		...
		if (canTouchEvent == true) {
			...
			} else if (hit(0, 23, 19, "up") == true) {
				damagePit(gp.dialogueState);
			} else if (hit(0, 23, 12, "up") == true) {
				healingPool(gp.dialogueState);
			} else if (hit(0, 10, 39, "any") == true) {
				teleport(1, 12, 13, gp.indoor);// to merchant's house
